...
function profileSummary(){
  ...
  return `<div class="card profile-card dashboard-profile-card">
    <div class="profile-avatar-box">
      <img class="profile-avatar-live" src="${has?avatarSrc():'assets/avatar-boy.webp'}" alt="Avatar" />
    </div>
...
function renderProfile(){
  ...
  const startCard = `<section class="card hero profile-start-card">
    <img src="assets/logo-pwa.png" alt="UPKK SmartKids" class="brand-logo-big app-logo-icon" />
  ...
  const editCard = profile.studentId ? `<section class="card simple-login-card">
    ...
    <div class="grid2"><button class="choice ${profile.avatar==='boy'?'active':''}" onclick="selectAvatar('boy')"><img class="avatar" src="assets/avatar-boy.webp">Lelaki</button><button class="choice ${profile.avatar==='girl'?'active':''}" onclick="selectAvatar('girl')"><img class="avatar" src="assets/avatar-girl.webp">Perempuan</button></div>
    <div style="height:12px"></div>
...
function renderProfileSwitcher(title='Senarai Pilihan Pelajar', showAdd=false){
  ...
  const rows = profiles.map(p=>{
    ...
    const avatar = p.avatar === 'girl' ? 'assets/avatar-girl.webp' : 'assets/avatar-boy.webp';
...
function renderSettings(){
  ...
  const addChildCard = window.__UPKK_ADD_STUDENT_FORM ? `<section class="card simple-login-card add-child-card">
    ...
    <label class="small"><b>Avatar Pelajar</b></label>
    <div style="height:8px"></div>
    <div class="grid2"><button class="choice ${draftChild.avatar==='boy'?'active':''}" onclick="selectNewStudentAvatar('boy')"><img class="avatar" src="assets/avatar-boy.webp">Lelaki</button><button class="choice ${draftChild.avatar==='girl'?'active':''}" onclick="selectNewStudentAvatar('girl')"><img class="avatar" src="assets/avatar-girl.webp">Perempuan</button></div>
    <div style="height:12px"></div>
    <label class="small"><b>Mode Tulisan</b></label>
    <div style="height:8px"></div>
    <div class="grid2"><button class="choice ${draftChild.mode!=='jawi'?'active':''}" onclick="selectNewStudentMode('rumi')"><div class="mode mode-rumi">Aa</div><b>RUMI</b></button><button class="choice ${draftChild.mode==='jawi'?'active':''}" onclick="selectNewStudentMode('jawi')"><div class="mode mode-jawi">ا ب</div><b>JAWI</b></button></div>
    <div style="height:14px"></div>
...
function chooseAnswer(i){
  ...
  if(quizType==='exam'){
    selectedAnswer=i; upsertExamAnswer(q,i); persistExamSession(); renderQuiz(); return;
  }
  if(selectedAnswer!==null) return; selectedAnswer=i; const correct=i===q.preparedAnswer; if(correct) currentQuiz.score++; currentQuiz.answers.push({id:q.id, sourceNo:q.sourceNo, correct}); renderQuiz();
}
...
function nextQuestion(){ if(quizType==='exam'){ if(currentQuiz.index<currentQuiz.questions.length-1){ currentQuiz.index++; selectedAnswer=selectedForCurrent(); persistExamSession(); renderQuiz(); } return; } selectedAnswer=null; if(currentQuiz.index<currentQuiz.questions.length-1){ currentQuiz.index++; renderQuiz(); return; } finishQuiz(); }
function prevQuestion(){ if(quizType!=='exam') return; if(currentQuiz.index>0){ currentQuiz.index--; selectedAnswer=selectedForCurrent(); persistExamSession(); renderQuiz(); } }
function confirmSubmitExam(){ appConfirm('Hantar jawapan peperiksaan sekarang?', ()=>finishQuiz(false)); }
